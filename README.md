...
Real RAGAS scores across all 12 configurations, generated by `python -m src.main ragas` over the 5 reference-answer gold queries. Uses the official `ragas` package with a `gpt-4o-mini` judge and OpenAI (`text-embedding-3-small`) embeddings. Context Precision and Context Recall are reference-grounded, so evaluation runs over the gold set (the queries that carry hand-written reference answers), not the 10 unlabeled `EVAL_QUERIES`. Corpus: the default AAPL/MSFT/GOOGL/NVDA/META 10-K/10-Q set (128 sections).

| Configuration | Faithfulness | Answer Relevancy | Context Precision | Context Recall | Composite |
| --- | --- | --- | --- | --- | --- |
| **semantic × hybrid_reranked** | **0.98** | **0.95** | **0.63** | **0.88** | **0.859** |
| semantic × hybrid | 0.97 | 0.95 | 0.61 | 0.76 | 0.822 |
| fixed × hybrid_reranked | 0.90 | 0.95 | 0.84 | 0.57 | 0.816 |
| semantic × sparse | 0.99 | 0.77 | 0.79 | 0.69 | 0.811 |
| fixed × hybrid | 0.99 | 0.77 | 0.76 | 0.57 | 0.771 |
| sentence × hybrid_reranked | 0.99 | 0.78 | 0.41 | 0.63 | 0.699 |
| fixed × dense | 0.97 | 0.76 | 0.45 | 0.33 | 0.629 |
| sentence × hybrid | 0.95 | 0.58 | 0.37 | 0.57 | 0.619 |
| fixed × sparse | 1.00 | 0.40 | 0.38 | 0.32 | 0.524 |
| semantic × dense | 0.96 | 0.38 | 0.43 | 0.28 | 0.512 |
| sentence × sparse | 0.89 | 0.40 | 0.23 | 0.28 | 0.451 |
| sentence × dense | 0.87 | 0.19 | 0.28 | 0.21 | 0.389 |

> *Composite = mean of the four metrics. Reproduce with `python -m src.main ragas` (requires `OPENAI_API_KEY`). Scores vary with the filings ingested and are computed on a 5-query gold set, so treat them as directional, not precise.*

**Correction (2026-09-12):** an earlier version of this table reported numbers that were **not produced by any code**: the README named RAGAS metrics but `benchmark.py` never called RAGAS. The table above is now a genuine RAGAS run wired in `src/evaluation/ragas_eval.py`. Several values changed materially (e.g. the top configuration's Context Precision is **0.63**, not the 0.85 previously shown), so the new numbers supersede the old ones entirely.

**Key finding:** Semantic chunking with hybrid retrieval tops the ranking (composite 0.859), and hybrid retrieval clearly beats dense- or sparse-only: the three weakest rows are all dense/sparse. Faithfulness is uniformly high (0.87–1.00) and barely discriminates; the spread is driven by answer relevancy and context precision/recall. Reranking helps, but **not** uniformly "3–5% across all metrics": its effect is metric- and chunking-dependent: it lifts fixed-chunking Context Precision 0.76 → 0.84, yet on semantic chunking it leaves precision roughly flat (0.61 → 0.63) while pushing Context Recall 0.76 → 0.88.
...
# Step 2b: Generate the RAGAS benchmark table (real ragas metrics over the gold set)
python -m src.main ragas
